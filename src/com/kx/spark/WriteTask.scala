...
class WriteTask(partitionId: Int, attemptNumber: Int, jobid: String, schema: StructType, mode: SaveMode, optionmap: Map[String,String]) 
      ...
      batch(i) match {
        ...
        case a:Array[Object] => a(batchind) = 
          schema(i).dataType match {
            case StringType => if (nn) row.getAs[String](i).toCharArray else Kdb.StringNull
            ...
            case Kdb.LongArrayType => row(i).asInstanceOf[WrappedArray[Array[Long]]].array
